...
public class InternetIndexController {
	...
	public String index(Locale locale, Model model) {
		...
		model.addAttribute("special", "internet");
...
//	}
   ...
   // 页面一加载就执行的方法
    ...
    @RequestParam(value="pageNumber") String pageNumber){
		String lsltype = request.getParameter("lsltype");
		String datetype = request.getParameter("datetype");	
		String pp = request.getParameter("pp");
		if("2".equals(pp)){
			if(!"".equals(lsltype) && lsltype!=null ){
			
				session.setAttribute("lsltype", lsltype);
				
			}
	        if(!"".equals(datetype) && datetype!=null){
		...
		if("1".equals(pp)){
			if(!"".equals(lsltype) && lsltype!=null ){
				session.removeAttribute("lsltype");
				session.setAttribute("lsltype", lsltype);
        ...
        //设置最大页数
        int totalCount=0;
        int count=cityKpi2019Service.getSizeInternetIndex(map1);
        if(count>0){
            totalCount=count;
        }
        ...
        //分页查询
        map=new HashMap();
        map.put("lsltype", lsltype);
		map.put("datetype", datetype);
		
        map.put("pageNo",tempPageNo);
        map.put("pageSize",pageSize);
        try {
        	List<Map> all = cityKpi2019Service.pageListInternetIndex(map);
...
//修改操作数据展示到模态框
	...
	public Map<String, Object> cityShowAndSet(Locale locale, Model model, HttpServletRequest request,
		...
		// 城市维度数据
		InternetIndex model5G =cityKpi2019Service.selectOneInternetIndex(i);
	...
	public int addAndUpdateLifeDate1(InternetIndex model5g, Locale locale, Model model, HttpServletRequest request,
			HttpServletResponse response) {
		
		System.out.println(model5g);
		Integer id = model5g.getId();
		int sum = 0;
		InternetIndex pm = new InternetIndex();
		try {
			InternetIndex model5G =cityKpi2019Service.selectOneInternetIndex(id);
			if (model5G == null) {
				// 添加
				@SuppressWarnings("unused")
				int insertLifeDate = cityKpi2019Service.insertInternetIndex(model5g);
			...
			if (model5G != null) {
				...
				@SuppressWarnings("unused")
				int updateLifeDate = cityKpi2019Service.updateOneInternetIndex(model5g);
	...
	//对期初数据进行删除操作
		...
		public int addAndU(BrokenScreen model5g, Locale locale, Model model, HttpServletRequest request,
			...
			try {
				int model5G =cityKpi2019Service.deleteOneInternetIndex(id);
				if(model5G==1){
