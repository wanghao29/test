  ...
  <div class="row" style="margin-top: 50px;">
	<div class="col-xs-12">
		...
		<table border="1" bordercolor="#1addfa" id="table5">
			...
			<tr  bgcolor="#4682B4">
			<td colspan="22" align="center" height="40px;"style="font-size: 25px;">新增  / 修改产品信息设置</td>
			</tr>
			...
			<td style="text-align: center;">
				...
				<td >
				<select name="term_mdl_nam"  id="select2" onchange="updateSetInformation()" class="select_searchableMy"  style="background-color: #00063c;border: none;width: 80px;"> 
...
/*  function keyPress(){
 ...
 /*对客户所选的品牌进行查询赋值操作，如果存在即赋值，不存在即为空 updateOne   */

 function  updateSetInformation(){
	 var term_brnd_nam=	$("#select1").val();
 	 var term_mdl_nam=	$("#select2").val();  
	 
 	 $.ajax({  
         url: "../productInformationSet/updateOne.do",  
         type: "post", 
         async: false,  
         cache: false,  
         dataType: "json",  
         data: {  
        	 'term_brnd_nam': term_brnd_nam,'term_mdl_nam':term_mdl_nam  //传入角色id，到后台获取json  
         },  
         success: function (result) {
        	 //document.getElementById("METERDES").innerHTML = result[0].meterdes;  
        	// $("#METERDES").html(result[0].meterdes);
        	 document.getElementById("METERDES").value = result[0].meterdes; 
        	 document.getElementById("METER").value = result[0].meter;
        	 document.getElementById("SNAME").value = result[0].sname;
        	 document.getElementById("PM").value = result[0].pm;
        	 var pricelevle=result[0].pricelevle;
        	 optionValue($("#PRICELEVLE"),pricelevle);
        	// document.getElementById("PRICELEVLE").innerHTML = "<option>"+ result[0].pricelevle+ "</option>";
        	 document.getElementById("PURPRICE").value = result[0].purprice;
        	 var proxytype=result[0].proxytype;
        	 optionValue($("#PROXYTYPE"),proxytype);
        	 var COPERMODEL=result[0].copermodel;
        	 optionValue($("#COPERMODEL"),COPERMODEL);
        	var COMPETBRANDNAME1=result[0].competbrandname1;
        	optionValue($("#COMPETBRANDNAME1"),COMPETBRANDNAME1);
