...
<head>
	...
	<script type="text/javascript">
		$(function(){
			...
			//初始化列表
			loadTable('#grid',
		            basePath + "/amount/getPersonList.do",
		            ['month','type8','dimension','areaname','people_num'],
		            [ '月份', '类型', '指标','地市','促销员人数'],
		            true);
			//查询
    ...
    $(table).bootstrapTable({
        ...
        onEditableSave: function (field, row, oldValue, $el) {
            $.ajax({
                type: "post",
                url: basePath + "/amount/updateByPerson.do",
	...
	function createCols(params,titles) {
        ...
        for(var i = 0;i<params.length;i++) {
            var obj = {};
            if(titles[i] == '促销员人数'){
                obj.editable={type: 'text', title: '编辑', validate:  function (v) {if(v=='0'){return '促销人数不能修改为0';}}}
            }
	...
	function addSubmit(){
		if(checkSubmit()){
			$.ajax({
	              type: "post",
	              url: basePath + "/amount/addPerson.do",
	              contentType: "application/json; charset=utf-8",
	              data: JSON.stringify({'addMonth':$('#addMonth').val(),'addType8':'量','addDimension':'人均销量','addCity':$('#addCity').val(),'addPeople_num':$('#addPeople_num').val()}),
	              dataType: 'JSON',
	              success: function (data, status) {
	                  if (status == "success") {
	                      if(!confirm("是否继续新增")){
	...
	function checkSubmit(){
		...
		var addPeople_num=$('#addPeople_num').val();
		if(addCity==null || addCity ==""){
			alert("[地市]不能为空");
			return false;
		}
		if(addPeople_num==null || addPeople_num ==""){
			alert("[促销人数]不能为空");
			return false;
		}
		//正则
		var reg=/^\d+$/;
		if(!reg.test(addPeople_num)){
			alert("[促销人数]必须为正整数");
			return false;
		}
		return true;
...
<div id="box_list">
	...
	<div id="box_add">
		...
		<br><br>
		<label style="color:blue;float:left;">促销员人数:</label>
		<div style="width:400px;float:left;"><input  id="addPeople_num" type="text" class="form-control"/></div>
		<br><br>
