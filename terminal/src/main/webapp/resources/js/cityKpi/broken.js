...
function updateNew() {  
	...
	var ecssAport =$("#model_DURATION1").val();
	var product=$("#model_DURATION2").val();
	var retail=$("#model_DURATION3").val();
	var basePrice =$("#model_DURATION4").val();
	var shopPrice=$("#model_DURATION5").val();
	if(ecssAport==""){
		alert("ECSS编码不能为空！");
		return false;
	}
   ...
   $.ajax({  
        type: "post",  
        url: "../broken/updateModel.do",  
       /*  data: "brand=" + brand + "&spec=" + spec,   */
       data:{"id":id,"ecssAport":ecssAport,"product":product,"retail":retail,"basePrice":basePrice,
    	   "shopPrice":shopPrice},
        dataType: 'json',  
...
function selelctAll(){
	...
	var ecssAport=$("#kind1").val();
	var product=$("#brand1").val();
	var pageNumber=1;
	var pp="1";
	$.StandardGet(basePath+'/broken/modelList.do',{'ecssAport':ecssAport,'product':product,'pageNumber':pageNumber,'pp':pp});
