...
function showPortrays(portrays,prefix,ids,shapeId,func){	
	if(ids==null || ids.length==0){
		$.each(portrays, function(index, cusChan) {
			...
			var billcyc_xfz = "billcyc_xfz"+(index+1);
			$("#"+shapeId+"_text").text(cusChan.cnName);
			var tip = isEmptyObject(cusChan)?"":((cusChan.tip==null)?"":cusChan.tip);
			$("#"+shapeId+"_text").attr("title",tip);
			$("#"+billcyc_xfz).text($("#statMo").val());
			
			$("#billcyc_zdtz"+(index+1)).text($("#statMo").val());
			$("#billcyc_jlfx"+(index+1)).text($("#statMo").val());
...
//			        }
//			        //计算百分比
//			        option.series[0].label.normal.formatter=function(params){return params.value+"\n"+(params.value/sum*100).toFixed(3)+"%";};
//			   }
	      ...
	      if( cusChan.showType==44){
			   ...
			   var name1 = detail_pre.split(',')[0];//存量
//			   var name2 = detail_pre.split(',')[1];//新增
...
//			   var option = oneBar2();	//存量  与 新增作对比的两类柱状图
			   var option = oneBar();	//单一柱状图
			   
			   option.xAxis[0].data = details;//x轴
			   
			   option.series[0].name = name1;	//存量
			   option.series[0].data = values1;	
			   
//			   option.series[1].name = name2;	//新增
