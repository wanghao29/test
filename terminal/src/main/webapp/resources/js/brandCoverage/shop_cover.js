...
function showNumsCover(months,cnts){  
	  var numPaint = initEcharts(echarts,"cover_num"); 
	  var numOption = buildLineAreaChart2();
	  $.each(months, function(idx, item) {
		  numOption.xAxis.data.push(item);
	  });
...
function brndTttt(brndTttt){
	...
	resetDiv($("#store_customer_number2"));
	 ...
	 var names = new Array('高价值门店','较高价值门店','低高价值门店','偏低价值门店','较低价值门店','低价值门店');
...
function showTermBrnd(arr){
    ...
    $("#"+id).append(`<option value="" selected>所有</option>`);
    $.each(arr,function(index,item){
    	//$("#"+id).append("<option value='"+''+"'>"+'所有'+"</option>"); 
    	$("#"+id).append("<option value='"+item+"'>"+item+"</option>"); 
//    	if(item=="苹果"){
//    		
//    		$("#"+id).append("<option value='"+item+"selected'>"+item+"</option>");
//    		
//    	}
    	
    	
    	
    	//$(".select2").trigger('change');//重新渲染
    });
