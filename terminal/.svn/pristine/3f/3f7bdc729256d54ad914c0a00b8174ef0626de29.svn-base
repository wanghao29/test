			...
			<div class="col-xs-1"> 	 				
		    	 ...
		    	 <div class="col-xs-1" >
 	 				   <div style="width:100%;position:absolute;">
					        <select name="cusOrgacode" id="cusOrgacode" runat="server" style="float:left;width: 100%;border-radius:22px;background-color:#00063a;height: 30px;border:1px solid #558baf;" class="select_searchableMy">
								<%-- <c:forEach var="item" items="${customers}">
								         <option value="${item.text}" <c:if test="${item.cusOrgacode==vo.cusOrgacode}"></c:if>
...
$(document).ready(function() {
	...
	//$(".startMonth").val(getStartMonthValue(0));
	   $("#startMonth").val("201910");
	   $("#endMonth").val("201910");
	//城市下拉框初始化
...
function jqgridInit(){
    var colNames = [ 
                ...
                '存量用户数','DM窜货量','客流量','预计换机用户数','男性数量','女性数量','arpu(元)','mou(分钟)','dou(M)',  
