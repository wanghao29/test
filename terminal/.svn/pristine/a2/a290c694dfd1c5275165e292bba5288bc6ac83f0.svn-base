...
function getLastData(){
	 $.ajax({
         ...
         contentType: "application/json; charset=utf-8",
		   ...
		   success: function(result,textStatus){
		       //设置上端的几个关键值
             var lr = result.lastData.lr;
             var htzygsl = result.lastData.htzygsl;
             var lwpqygsl = result.lastData.lwpqygsl;
             var rs = result.lastData.rs;
             var rjlr = result.lastData.rjlr;
             $('#hLr').text(lr);
