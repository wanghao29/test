...
<head>
	...
	<script type="text/javascript">
		$(function(){
			...
			//初始化列表
			loadTable('#grid',
		            basePath + "/amount/getCostList.do",
		            ['month','type8','dimension','kpiName','areaname','commission','cost'],
		            [ '月份', '类型', '指标','四大野战军','地市','佣金','费用'],
		            true);
			//查询
			$("#btnSearch").click(function(){
				...
				if($("#kpiSelect").val() != null && $("#kpiSelect").val() != ""){
					$("#search_hid").val(1);
				}
    ...
    $(table).bootstrapTable({
        ...
        onEditableSave: function (field, row, oldValue, $el) {
            $.ajax({
                type: "post",
                url: basePath + "/amount/updateByCost.do",
	...
	function addSubmit(){
		if(checkSubmit()){
			$.ajax({
	              type: "post",
	              url: basePath + "/amount/addCost.do",
	              contentType: "application/json; charset=utf-8",
	              data: JSON.stringify({'addMonth':$('#addMonth').val(),'addType8':'利','addDimension':'净利','addKpiName':$('#addKpiName').val(),'addCity':$('#addCity').val(),'addCommission':$('#addCommission').val(),'addCost':$('#addCost').val()}),
	              dataType: 'JSON',
	              success: function (data, status) {
	                  if (status == "success") {
	                      if(!confirm("是否继续新增")){
	...
	function checkSubmit(){
		...
		if(addCommission==null || addCommission ==""){
			alert("[酬金]不能为空");
			return false;
		}
		if(addCost==null || addCost ==""){
			alert("[佣金]不能为空");
			return false;
		}
		//正则
		var reg=/^[0-9]{1,9}(\.[0-9]{0,9})?$/;
		if(!reg.test(addCommission)){
			alert("[酬金]必须为数字");
			return false;
		}
		if(!reg.test(addCost)){
...
<div id="box_list">
	<form class="form-horizontal" id="form_dialog" method="post" >
		...
		<div style="float:left">
			...
			<select name="tag1" class="select" style="width: 100%;color:blue;" id="kpiSelect">
														<option value="" >请选择战区</option>
				 	 				   		       		<option value="东莞运营中心">东莞运营中心</option>
				 	 				   		       		<option value="佛山运营中心">佛山运营中心</option>
				 	 				   		       		<option value="广州运营中心">广州运营中心</option>
				 	 				   		       		<option value="深圳运营中心">深圳运营中心</option>
			</select>
		</div>
		<div style="float:left;width:150px;">
	...
	<!-- '月份', '类型', '指标','四大野战军','地市','佣金','费用' -->
		...
		<br><br>
		<label style="color:blue;">四大野战军:</label>
		<select class="select" style="width: 30%;color:blue;" id="addKpiName">
														<option value="" >请选择战区</option>
				 	 				   		       		<option value="东莞运营中心">东莞运营中心</option>
				 	 				   		       		<option value="佛山运营中心">佛山运营中心</option>
				 	 				   		       		<option value="广州运营中心">广州运营中心</option>
		...
		<br><br>
		<label style="color:blue;float:left;">佣金:</label>
		<div style="width:400px;float:left;"><input  id="addCommission" type="text" class="form-control"/></div>
		<br><br>
		<label style="color:blue;float:left;">费用:</label>
		<div style="width:400px;float:left;"><input  id="addCost" type="text" class="form-control"/></div>
		<br><br>
