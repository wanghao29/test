$(document).ready(function() {
	...
	// $("#cm_brand").select2();
	// $("#lm_brand").select2();
	 $("#pkgDtlFeeAmtMin").select2();
	 $("#pkgDtlFeeAmtMax").select2();
	// $("#cm_ter_type").select2();
	 $("#lm_ter_type").select2();
	//级联的change事件
	 $("#cm_brand").on("change",specSelect);
	 //$("#lm_brand").on("change",beforeSpecSelect);
	 $("#pkgDtlFeeAmtMin").on("change",function specSelect(){
		 var pkgDtlFeeAmtMin = $("#pkgDtlFeeAmtMin").val();
		 $("#pkgDtlFeeAmtMax").val(pkgDtlFeeAmtMin).select2();
	 });
	 $("#pkgDtlFeeAmtMax").on("change",function specSelect(){
		    var pkgDtlFeeAmtMin = $("#pkgDtlFeeAmtMin").val();
			var pkgDtlFeeAmtMax	= $("#pkgDtlFeeAmtMax").val();
			if(pkgDtlFeeAmtMax !="" && pkgDtlFeeAmtMax !=null){
				if(Number(pkgDtlFeeAmtMin) >Number(pkgDtlFeeAmtMax) ){
					alert("当前主套餐价格不能小于:"+pkgDtlFeeAmtMin);
					$("#pkgDtlFeeAmtMax").val(pkgDtlFeeAmtMin).select2();
				}
			}
...
//	var startDay = year  + month  ;
	...
	$("#searchBtn").click(function() {
	  ...
	  var postData = $('#list').jqGrid("getGridParam", "postData");
	   ...
	   queryParams["pkgDtlFeeAmtMin"]=$("#pkgDtlFeeAmtMin").val();
	   queryParams["pkgDtlFeeAmtMax"]=$("#pkgDtlFeeAmtMax").val();
	   ...
	   jqgridReload(queryParams);
 		...
 		$.ajax({
		       //几个参数需要注意一下
		           ...
		           url: basePath+'/consumerBigData/sumInnetUsrCnt.do' ,//url
		           data:  queryParams,
		           async: false,
		           success: function (result) {
		        	   $("#total_user").text("用户数 : "+result.return.InnetUsrCnt);
		           },
		           error : function() {
		        	   alert("false");
		           }
	  ...
	  jQuery("#list").jqGrid(
	      {
	        ...
	        //scroll:1, 滚动加载
	        //datatype : "local",
	        colNames : [ '月份','号码','客户ID','厅店ID','地市','区域_市','区域_县','号码品牌','性别','籍贯',
	        	...
	        	'当前终端品牌','当前终端型号','当前终端产品型号' ,'换机前的终端品牌', '换机前的终端型号','终端价格', '手机分辨率', '内存大小','终端重量',
	        	'屏幕大小','电池容量','摄像头像素' ,'是否双卡双待', '网络制式', '网络类型','是否山寨机',
	        	'是否智能机','是否移动购机','购机类型' ,'是否当月新增终端','是否当月新入网用户','是否支持nfc功能', '是否支持wifi', '是否支持指纹解锁','是否双摄像头',
	        	'是否旗舰机','是否ifTd','是否ifTdd' ,'终端机龄', '换机月份', '换机周期','换机次数',
	        	'换机类型','换机类型2','换机前的imei' , '换机前的终端产品型号','换机前的终端价格',
	        	'换机前的手机分辨率','换机前的内存大小','换机前的终端重量大小' ,'换机前的屏幕大小', '换机前的电池容量', '换机前的摄像头像素','客户数','在网用户数',
	        	'主动通信用户数','总消费额','总流量费用' ,'总计费时长', '总流量', '总4g流量','点对点短信上行条数',
	        	'aou','可用帐户总余额' ,'主套餐价格','平均机龄','平均换机周期'
	        	],
	        colModel : [ 
                 ...
                 {name : 'newTermInd',index : 'newTermInd'  }, 
                 {name : 'newUsrInd',index : 'newUsrInd'  }, 
                 ...
                 {name : 'custCnt',index : 'custCnt' ,hidden:true }, 
                 ...
                 {name : 'pkgDtlFeeAmt',index : 'pkgDtlFeeAmt'  },
                 {name : 'avgJiAgeRak',index : 'avgJiAgeRak'  },
                 {name : 'avgHuanjiZqRnk',index : 'avgHuanjiZqRnk'  }
                 /*, 
...
function brandSelect(){
 	$.ajax({
	       //几个参数需要注意一下
	           type: 'post',//方法类型
	           dataType: "json",//预期服务器返回的数据类型
	           url: basePath+'/consumerBigData/pkgDtlFeeAmt.do' ,//url
	           data: null,
	           async: false,
	           contentType: false,  
	           processData: false, 
	           success: function (result) {
	        	  // $("#cm_brand").append("<option>全部</option>");
	        	   //$("#lm_brand").append("<option>全部</option>");
	        	   $("#pkgDtlFeeAmtMin").append("<option value=''>全部</option>");
	        	   $("#pkgDtlFeeAmtMax").append("<option  value=''>全部</option>");
	        	 
	        	  var pkgDtlFeeAmt = result.pkgDtlFeeAmt;
	        	 
	        	  for (var i = 0; i < pkgDtlFeeAmt.length; i++) {
						$("#pkgDtlFeeAmtMin").append("<option value="+pkgDtlFeeAmt[i]+">"+pkgDtlFeeAmt[i]+"</option>");// 
						$("#pkgDtlFeeAmtMax").append("<option value="+pkgDtlFeeAmt[i]+">"+pkgDtlFeeAmt[i]+"</option>");// 
	        	  }
	           },
	           error : function() {
	        	   alert("false");
	 ...
	 $('#lm_brand').select2({
		    ...
		    allowClear: true,
		    ajax : {
		     url :basePath+'/consumerBigData/brandSelect.do'  ,
		     dataType : 'json',
		     delay : 250,// 延迟显示k
		    async: false, 
		     data : function(params) {
		      return {
		       cm_brand : params.term, // 搜索框内输入的内容，传递到Java后端的parameter为username
	...
	/* 	     markup += "<div class='select2-result-repository__statistics'>"
		    ...
		    $("#cm_brand").off("select2:select").on("select2:select",function(e){ 
		    	specSelect();
			});
...
function specSelect(){
	...
	/* $.ajax({
	       //几个参数需要注意一下
	           ...
	           success: function(result){
	        	   $("#cm_ter_type").empty();
	        	   $("#cm_ter_type").append("<option>全部</option>");
	...
	$('#cm_ter_type').select2({
	    ...
	    allowClear: true,
	    ajax : {
	     url :basePath+'/consumerBigData/specSelect.do'  ,
	     dataType : 'json',
	     delay : 250,// 延迟显示k
	/*     async: false,*/
	     data : function(params) {
	      return {
	       cm_ter_type : params.term, // 搜索框内输入的内容，传递到Java后端的parameter为username
	       page : params.page,// 第几页，分页哦
	       rows : 10,// 每页显示多少行
	       statMo : $("#billcyc").val(),
	       city: $("#citySelect").val(),
	       cm_brand: $("#cm_brand").val(),
	      // cmccCntyNam: $("#cmccCntyNam").val(),
	      // tableName: dataBaseTable+$("#statMo").val(),//'ZD_RESULT_CONSUMER_ZL_XFZ_201901'
	     //  brndCd:$("#brndCd").val(),
	      };
