...
@Controller
@RequestMapping("/ownBrandIncome")
public class OwnBrandIncomeController {

	@Resource
	private IOwnBrandIncomeService ownBrandIncomeService;
	...
	@DS("dataSource")@RequestMapping(value = "/index", method = RequestMethod.GET)
   	public String newIndex(HttpServletRequest request, Locale locale, Model model, String special, String billcyc) {
   		...
   		return "gfKpi/operateIncome/ownBrandIncome";
   	}
	...
	public JSONObject query(HttpServletRequest req, HttpServletResponse res) {
		...
		try {
			ownBrandIncomeService.calculatedScore(String.valueOf(params.get("startMonth")), String.valueOf(params.get("endMonth")));
			List<OwnBrandIncome> outList = new ArrayList<OwnBrandIncome>();
			PageInfo<OwnBrandIncome> pageInfo =ownBrandIncomeService.queryData(params);
