...
public class TotalProfitController {
	@Resource
	private ITotalProfitService totalProfitService;
	...
	@DS("dataSource")@RequestMapping(value = "/index", method = RequestMethod.GET)
   	public String newIndex(HttpServletRequest request, Locale locale, Model model, String special, String billcyc) {
   		...
   		return "gfKpi/operateBenefit/totalProfit";
   	}
	...
	public JSONObject query(HttpServletRequest req, HttpServletResponse res) {
		...
		try {
			totalProfitService.calculatedScore(String.valueOf(params.get("startMonth")), String.valueOf(params.get("endMonth")));
			List<TotalProfit> outList = new ArrayList<TotalProfit>();
			PageInfo<TotalProfit> pageInfo =totalProfitService.queryData(params);
