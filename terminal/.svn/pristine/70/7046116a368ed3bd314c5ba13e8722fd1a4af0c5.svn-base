...
function jqgridInit(){
	  jQuery("#grid").jqGrid(
	      {
	        url:basePath+"/receivableTurnoverRate/queryData.do",
	        mtype:'post',
	        datatype: "json",
	        ...
	        //scroll:1, 滚动加载
	        //datatype : "local",
	        colNames : [ '月份','运营收入','省间调拨','运营收入（只剔除省间调拨）','应收帐款净值','本月应收款周转率','年度应收款周转率','目标值','得分' ,'年度得分' ],
	        colModel : [ 
	           {name : 'billcyc',index : 'billcyc' }, 
			   {name : 'yysr',index : 'yysr', formatter: "number", formatoptions: {thousandsSeparator:",", defaulValue:"",decimalPlaces:2}  }, 
			   {name : 'sjdb',index : 'sjdb', formatter: "number", formatoptions: {thousandsSeparator:",", defaulValue:"",decimalPlaces:2}  }, 
               {name : 'yysrkhsr',index : 'yysrkhsr', formatter: "number", formatoptions: {thousandsSeparator:",", defaulValue:"",decimalPlaces:2}  }, 
               {name : 'yszkjz',index : 'yszkjz', formatter: "number", formatoptions: {thousandsSeparator:",", defaulValue:"",decimalPlaces:2}  }, 
               {name : 'byyskzzl',index : 'byyskzzl', formatter: "number", formatoptions: {thousandsSeparator:",", defaulValue:"",decimalPlaces:2}  }, 
               {name : 'ndyskzzl',index : 'ndyskzzl',hidden:true, formatter: "number", formatoptions: {thousandsSeparator:",", defaulValue:"",decimalPlaces:2}  }, 
               {name : 'mbz',index : 'mbz', formatter: "number", formatoptions: {thousandsSeparator:",", defaulValue:"",decimalPlaces:2}  }, 
               {name : 'df',index : 'df'  },
...
function assignLableValue(rowData){
	...
	$("#yszkjz").text(formatThousandsSeparator(rowData.yszkjz));
	$("#byyskzzl").text(rowData.byyskzzl);
	$("#df").text(rowData.df);
