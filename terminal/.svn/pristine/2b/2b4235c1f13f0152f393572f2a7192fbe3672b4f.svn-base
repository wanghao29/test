...
//    setAutoMonth();
    ...
    getEchartData();
        ...
        loadTable('#grid',
            basePath + "/corporationCollaboration/terminalSaleShare/list.do",
            ['month','wsso','jhl','tzxl','xlzb','df'],
            ['月份','我司SO','激活量','调整销量','销量占比','得分'],
            true);
...
//            console.log($(field[0]));
        ...
        onEditableSave: function (field, row, oldValue, $el) {
            if(field == 'tzxl'){
                $.ajax({
                    type: "post",
                    url: basePath + "/corporationCollaboration/terminalSaleShare/updateByTargetVal.do",
                    contentType: "application/json; charset=utf-8",
                    data: JSON.stringify(row),//JSON.stringify({"startMonth":$("input[name='startMonth']").val(),"endMonth":$("input[name='endMonth']").val(),"mbz":row.tzxl}),
                    dataType: 'JSON',
...
$.ajax({
    type: "post",
    url: basePath + "/corporationCollaboration/terminalSaleShare/updateByPrimaryKey.do",
    contentType: "application/json; charset=utf-8",
...
for(var i = 0;i<params.length;i++) {
    var obj = {};
//    if(titles[i] == '调整销量'){
//        obj.editable={type: 'text', title: '编辑', validate:  function (v) {if(v=='0'){return '调整销量不能修改为0';}}}
//    }
//    else if(titles[i] == '得分'){
...
function getEchartData(){
	$.ajax({
		   type: 'post',
		   url: basePath+'/corporationCollaboration/terminalSaleShare/getEchartData.do',
        contentType: "application/json; charset=utf-8",
		   ...
		   success: function(result,textStatus){
		       //设置上端的几个关键值
            var lastXlzb = result.lastData.xlzb;
            var lastDf = result.lastData.df;
            $('#hXlzb').text(lastXlzb);
            $('#hDf').text(lastDf);
            //初始化图表
