...
//    setAutoMonth();
    ...
    getLastData();
        ...
        var option = threeGauge2(currentMonthWcqkzb,lastMonthWcqkzb,lastYearWcqkzb,'年度应收款周转率');
        ...
        loadTable('#grid',
            ...
            ['month','yysr','sgtzxm','sjdb','yszkkhsr','yszkjz','byyskzzl','ndyskzzl','mbz','df'],
            ['月份','运营收入','手工调整项目','省间调拨','应收账款考核收入','应收帐款净值','本月应收款周转率','年度应收款周转率','目标值','得分'],
            true);
...
//                    console.log($(field[0]));
                ...
                onEditableSave: function (field, row, oldValue, $el) {
	                    ...
	                    $.ajax({
	                        type: "post",
	                        url: basePath + "/assetsTurnoverRate/accountReceivableTurnoverRatio/updateByReceivePayment.do",
        ...
        function getLastData(){
        	$.ajax({
     		   type: 'post',
     		   url: basePath+'/assetsTurnoverRate/accountReceivableTurnoverRatio/getLastData.do',
                contentType: "application/json; charset=utf-8",
     		   data: JSON.stringify({endMonth:$("#endMonth").val()}),
     		   async: false,
     		   dataType: 'json',
     		   success: function(result){
     			  /* if(Math.abs(result.byyskzzl/100000000)>1){
     				  $('#hByyskzzl').text((result.byyskzzl/100000000)+"亿");
     			   }else if(Math.abs(result.byyskzzl)/10000>1){
     				  $('#hByyskzzl').text(result.byyskzzl/10000+"万");
     			   }else{
     				 
     			   }
     		       
     		      if(Math.abs(result.ndyskzzl/100000000)>1){
     		    	 $('#hNdyskzzl').text((result.ndyskzzl/100000000)+"亿");
    			   }else if(Math.abs(result.ndyskzzl/10000)>1){
    				   $('#hNdyskzzl').text(result.ndyskzzl/10000+"万");
    			   }else{
    				  
    			   }
                    */
     			  $('#hByyskzzl').text(result.byyskzzl);
     			  $('#hNdyskzzl').text(result.ndyskzzl);
                   $('#hDf').text(result.df);
     		   }
     	   });
        }
...
//            }
//            else if(titles[i] == '应收帐款净值'){
//                obj.editable={type: 'text', title: '编辑', validate:  function (v) {}}
//            }
//            else if(titles[i] == '目标值'){
//                obj.editable={type: 'text', title: '编辑', validate:  function (v) {
//                    if(v=='0'){return '目标值不能修改为0';
//                }}}
//            }
            obj.field = params[i];
            obj.title = titles[i];
    ...
    function getWcqkzbs(row) {
        $.ajax({
            type: "post",
            url: basePath + "/assetsTurnoverRate/accountReceivableTurnoverRatio/getWcqkzbs.do",
            contentType: "application/json; charset=utf-8",
            data: JSON.stringify(row),
            dataType: 'JSON',
