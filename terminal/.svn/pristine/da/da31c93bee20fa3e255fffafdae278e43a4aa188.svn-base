...
@Controller
@RequestMapping("/InternetCome")
public class InternetIncomeController {
	...
	public String index(Locale locale, Model model) {
		...
		return "cityKpi/model_internetIncome";
	}
	...
	//汇总KPI指标操作
		...
		public Map<String, Object> addAndU(InternetIndex modelI, Locale locale, Model model, HttpServletRequest request,
		        ...
		        if(kind.equals("1")){ //代表是月份  
		        	...
		        	try {
						//互联网完成情况  (参数可以是月份和日期)
		        		String yy = cityKpi2019Service.selectDateIncome(datevalue, end);
						//互联网考核指标（参数必须是月份，需转换）
						String tt = cityKpi2019Service.selectDateIIncome(modelI);  
				...
				String topYear = format1.format(dateEnd)+"01";//上月开始时间
				 ...
				 try {
	            		  ...
	            		  if(timeKind!=null && "2".equals(timeKind)){
	            			  List<ModelInternet> listBig = cityKpi2019Service.listBigIncome(start, end);//当前日期大类 必须传日期格式的
		            		  if(listBig.size()>0){
		              			for(ModelInternet abo : listBig) {
		              				String fiveName = abo.getType();//手机类型    				
		              				//Double a = Double.valueOf(abo.getSaleincom());//销量
		              				list.add(fiveName);
		              				
		              				
		              				innerMap.put(fiveName,Long.parseLong(abo.getSaleincom()));	
	            		  ...
	            		  List<ModelInternet> listBig1 = cityKpi2019Service.listBigIncome(topDate, nextDate);//上月大类
	                      ...
	                      if(listBig1.size()>0){
	              			for(ModelInternet abo : listBig1) {
	              				...
	              				outterMap.put(fiveName,Long.parseLong(abo.getSaleincom()));	
	                      ...
	                      List<ModelInternet> listBig2 = cityKpi2019Service.listBigIncome(topYear, nextYear);//去年同期大类 
	                      //去年同期
	          			if(listBig2.size()>0){
		...
		//长方形品牌完成情况大类取数
				...
				public Map<String, Object> areaBrand(Locale locale, Model models, HttpServletRequest request,
		            ...
		            try {
		            	...
		            	List<ModelInternet> listBig = cityKpi2019Service.listBrandIncome(mdl);//当前日期大类 必须传日期格式的
		            	
		            	
		                if(listBig.size()>0){
		    			for(ModelInternet abo : listBig) {
		    				String fiveName = abo.getBrand();//手机品牌	
		    				String string = abo.getSaleincom();//数量
		    				//Double a = Double.valueOf(abo.getSaleincom());//销量
		    				list.add(fiveName);
		    				list2.add(string);
		    			}
		                ...
		                //上月大类
		            	List<ModelInternet> listBig1 = cityKpi2019Service.listBrandIncome(topMonth);//上月大类
		                if(listBig1.size()>0){
		                	for(ModelInternet abo : listBig1) {
			    				String fiveName = abo.getBrand();//手机品牌	
			    				String string = abo.getSaleincom();//数量
...
//			    			 map.put("monthAmount", month2);
		                    ...
		                    //去年同期
		                	List<ModelInternet> listBig2 = cityKpi2019Service.listBrandIncome(netYear);//去年同期大类
		        			if(listBig2.size()>0){
		        				for(ModelInternet abo : listBig2) {
				    				String fiveName = abo.getBrand();//手机品牌	
				    				String string = abo.getSaleincom();//数量
...
//					    			 map.put("yearAmount", year2);
				...
				public Map<String, Object> areaspec(Locale locale, Model models, HttpServletRequest request,
		            ...
		            try {
		            	...
		            	List<ModelInternet> listBig = cityKpi2019Service.listSpecIncome(mdl);//当前日期大类 必须传日期格式的
		                ...
		                //上月大类
		            	List<ModelInternet> listBig1 = cityKpi2019Service.listSpecIncome(topMonth);//上月大类
		                if(listBig1.size()>0){
		                	for(ModelInternet abo : listBig1) {
			    				String fiveName = abo.getSpec();//手机品牌	
			    				String string = abo.getSaleincom();//数量
...
//			    			 map.put("monthAmount", month2);
		                    ...
		                    //去年同期
		                	List<ModelInternet> listBig2 = cityKpi2019Service.listSpecIncome(netYear);//去年同期大类
		        			if(listBig2.size()>0){
		        				for(ModelInternet abo : listBig2) {
				    				String fiveName = abo.getSpec();//手机品牌	
				    				String string = abo.getSaleincom();//数量
...
//					    			 map.put("yearAmount", year2);
				...
				public Map<String, Object> areaPZ(Locale locale, Model models, HttpServletRequest request,
		            ...
		            try {
		            	...
		            	List<ModelInternet> listBig = cityKpi2019Service.listMemoryIncome(mdl);//当前日期大类 必须传日期格式的
		                ...
		                //上月大类
		            	List<ModelInternet> listBig1 = cityKpi2019Service.listMemoryIncome(topMonth);//上月大类
		                if(listBig1.size()>0){
		                	for(ModelInternet abo : listBig1) {
			    				String fiveName = abo.getMemory();//手机品牌	
			    				String string = abo.getSaleincom();//数量
...
//			    			 map.put("monthAmount", month2);
		                    ...
		                    //去年同期
		                	List<ModelInternet> listBig2 = cityKpi2019Service.listMemoryIncome(netYear);//去年同期大类
		        			if(listBig2.size()>0){
		        				for(ModelInternet abo : listBig2) {
				    				String fiveName = abo.getMemory();//手机品牌	
				    				String string = abo.getSaleincom();//数量
              ...
              //对颜色进行取数
				...
				public Map<String, Object> areaColor(Locale locale, Model models, HttpServletRequest request,
		            ...
		            try {
		            	...
		            	List<ModelInternet> listBig = cityKpi2019Service.listColorIncome(mdl);//当前日期大类 必须传日期格式的
		                ...
		                //上月大类
		            	List<ModelInternet> listBig1 = cityKpi2019Service.listColorIncome(topMonth);//上月大类
		                if(listBig1.size()>0){
		                	for(ModelInternet abo : listBig1) {
			    				String fiveName = abo.getColor();//手机品牌	
			    				String string = abo.getSaleincom();//数量
...
//			    			 map.put("monthAmount", month2);
		                    ...
		                    //去年同期
		                	List<ModelInternet> listBig2 = cityKpi2019Service.listColorIncome(netYear);//去年同期大类
		        			if(listBig2.size()>0){
		        				for(ModelInternet abo : listBig2) {
				    				String fiveName = abo.getColor();//手机品牌	
				    				String string = abo.getSaleincom();//数量
  ...
  //对渠道进行取数
				...
				public Map<String, Object> areaClient(Locale locale, Model models, HttpServletRequest request,
		            ...
		            try {
		            	...
		            	List<ModelInternet> listBig = cityKpi2019Service.listClientNameIncome(mdl);//当前日期大类 必须传日期格式的
		                ...
		                //上月大类
		            	List<ModelInternet> listBig1 = cityKpi2019Service.listClientNameIncome(topMonth);//上月大类
		                if(listBig1.size()>0){
		                	for(ModelInternet abo : listBig1) {
			    				String fiveName = abo.getClientname();//手机品牌	
			    				String string = abo.getSaleincom();//数量
...
//			    			 map.put("monthAmount", month2);
		                    ...
		                    //去年同期
		                	List<ModelInternet> listBig2 = cityKpi2019Service.listClientNameIncome(netYear);//去年同期大类
		        			if(listBig2.size()>0){
		        				for(ModelInternet abo : listBig2) {
				    				String fiveName = abo.getClientname();//手机品牌	
				    				String string = abo.getSaleincom();//数量
				...
				public List<StockTurnOver> doSomeThings(HttpServletRequest request,HttpServletResponse response) throws IOException, ParseException  {
		          ...
		          // String nextYear = format.format(dateYear);
		           ...
		           if(qq==null || "".equals(qq)){
		        	   String time ="20200101";
		        	   listAllStock = cityKpi2019Service.listAllStockIncome(time);  
		           }
		           if(qq!=null && !"".equals(qq)){
