...
//    setAutoMonth();
    ...
    $('#addItem').click(function () {
        //ction="${pageContext.request.contextPath }/assetsTurnoverRate/repaymentAccuracyForecast/addAndEdit.do

        BootstrapDialog.show({
            title: '新增回款预测准确度',
            message: $('<div></div>').load(basePath + '/assetsTurnoverRate/repaymentAccuracyForecast/dialog/getDialog.do',{limit: 25}, function(data){

            }),
            buttons: [{
                label: '确定',
                action: function(dialog) {
                    addRepaymentAccuracy();
                    dialog.close();
                }
            }, {
                label: '取消',
...
function addRepaymentAccuracy(){
    // var formData = new FormData(document.getElementById("addForm"));
    // console.log(formData);
    $.ajax({
        url:basePath + "/assetsTurnoverRate/repaymentAccuracyForecast/addAndEdit.do",
        type:"post",
...
//            console.log($(field[0]));
        ...
        onEditableSave: function (field, row, oldValue, $el) {
            $.ajax({
                type: "post",
                url: basePath + "/assetsTurnoverRate/repaymentAccuracyForecast/updateByReceivePayment.do",
...
$.ajax({
    type: "post",
    url: basePath + "/assetsTurnoverRate/repaymentAccuracyForecast/getWcqkzbs.do",
    contentType: "application/json; charset=utf-8",
    data: JSON.stringify(row),
    dataType: 'JSON',
    success: function (data) {
...
function getLastData(){
	$.ajax({
		   type: 'post',
		   url: basePath+'/assetsTurnoverRate/repaymentAccuracyForecast/getLastData.do',
        contentType: "application/json; charset=utf-8",
		   data: JSON.stringify({endMonth:$("input[name='endMonth']").val()}),
		   async: false,
		   dataType: 'json',
		   success: function(result){
		       $('#hHkyczqd').text(result.hkyczqd);
            $('#hHkyczqddf').text(result.hkyczqddf);
		   }
	   });
}
...
function initEchart(currentMonthWcqkzb,lastMonthWcqkzb,lastYearWcqkzb){
    ...
    var option = threeGauge2(currentMonthWcqkzb,lastMonthWcqkzb,lastYearWcqkzb,'回款预测准确度');
