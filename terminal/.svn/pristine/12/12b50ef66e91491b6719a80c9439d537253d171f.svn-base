...
function jqgridInit(){
	  jQuery("#grid").jqGrid(
	      {
	        url:basePath+"/totalProfit/queryData.do",
	        mtype:'post',
	        datatype: "json",
	        ...
	        colNames : [ '月份','总支出','互联网支出','考核支出','财务报表运营收入','互联网收入','省间调拨收入','互联网省间调拨收入','考核收入','业务成本','互联网成本','省间调拨成本' 
	        	,'互联网省间调拨成本','运营毛利','互联网毛利','省间调拨毛利','综合毛利','利润总额','全年完成利润总额','广分考核利润','基础目标值','全年完成度','时间完成度','得分','广分利润累计'
	        ...
	        colModel : [ 
               ...
               {name : 'qnwclrze',index : 'qnwclrze', formatter: "number", formatoptions: {thousandsSeparator:",", defaulValue:"",decimalPlaces:2}  }, 
               ...
               {name : 'jcmbz',index : 'jcmbz', formatter: "number", formatoptions: {thousandsSeparator:",", defaulValue:"",decimalPlaces:2}  } ,
               {name : 'qnwcd',index : 'qnwcd',hidden:true }, 
               {name : 'sjwcd',index : 'sjwcd'  }, 
               {name : 'df',index : 'df'  }, 
...
function assignLableValue(rowData){
	...
	$("#qnwclrze").text(formatThousandsSeparator(rowData.gfAssessmentProfitNd));
	$("#qnwcd").text(rowData.qnwcd);
	$("#sjjd").text(rowData.sjwcd+"%");
