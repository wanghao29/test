...
@Controller
@RequestMapping("/modelInternet")
public class ModelInternetController {
	...
	public String index(Locale locale, Model model) {
		...
		return "cityKpi/model_internet";
	}
	@RequestMapping(value = "/indexin", method = RequestMethod.GET)
	public String indexSet(Locale locale, Model model) {
		// logger.info("customerChannel {}.", locale);
		String special="type_8";
		String type_8="存";
		model.addAttribute("type_8", type_8);
		model.addAttribute("special", special );
		return "cityKpi/model_setIn";
	}
...
//		        	modelI.setDatevalue(start);
		        	 try {
		        	  String yy = cityKpi2019Service.selectDate(datevalue, end);
		        	
		        	
						//互联网完成情况  (参数可以是月份和日期)
						
						String tt = cityKpi2019Service.selectDateI(modelI);
				...
				String topYear = format1.format(dateEnd)+"01";//上月开始时间
				 try {
	            		  if(timeKind!=null && "2".equals(timeKind)){
	            			  List<ModelInternet> listBig = cityKpi2019Service.listBig(start, end);//当前日期大类 必须传日期格式的
		            		  if(listBig.size()>0){
		              			for(ModelInternet abo : listBig) {
		              				String fiveName = abo.getType();//手机类型    				
		              				//Double a = Double.valueOf(abo.getSaleamount());//销量
		              				list.add(fiveName);
		              				
		              				
		              				innerMap.put(fiveName,Long.parseLong(abo.getSaleamount()));	
	            		  ...
	            		  List<ModelInternet> listBig1 = cityKpi2019Service.listBig(topDate, nextDate);//上月大类
	                      ...
	                      if(listBig1.size()>0){
	              			for(ModelInternet abo : listBig1) {
	              				...
	              				outterMap.put(fiveName,Long.parseLong(abo.getSaleamount()));	
	                      ...
	                      List<ModelInternet> listBig2 = cityKpi2019Service.listBig(topYear, nextYear);//去年同期大类 
	                      //去年同期
	          			if(listBig2.size()>0){
		...
		//长方形品牌完成情况大类取数
				...
				public Map<String, Object> areaBrand(Locale locale, Model models, HttpServletRequest request,
		            ...
		            try {
		            	...
		            	List<ModelInternet> listBig = cityKpi2019Service.listBrand(mdl);//当前日期大类 必须传日期格式的
		            	
		            	
		                if(listBig.size()>0){
		    			for(ModelInternet abo : listBig) {
		    				String fiveName = abo.getBrand();//手机品牌	
		    				String string = abo.getSaleamount();//数量
		    				//Double a = Double.valueOf(abo.getSaleamount());//销量
		    				list.add(fiveName);
		    				list2.add(string);
		    			}
		                ...
		                //上月大类
		            	List<ModelInternet> listBig1 = cityKpi2019Service.listBrand(topMonth);//上月大类
		                if(listBig1.size()>0){
		                	for(ModelInternet abo : listBig1) {
			    				String fiveName = abo.getBrand();//手机品牌	
			    				String string = abo.getSaleamount();//数量
...
//			    			 map.put("monthAmount", month2);
		                    ...
		                    //去年同期
		                	List<ModelInternet> listBig2 = cityKpi2019Service.listBrand(netYear);//去年同期大类
		        			if(listBig2.size()>0){
		        				for(ModelInternet abo : listBig2) {
				    				String fiveName = abo.getBrand();//手机品牌	
				    				String string = abo.getSaleamount();//数量
...
//					    			 map.put("yearAmount", year2);
				...
				public Map<String, Object> areaspec(Locale locale, Model models, HttpServletRequest request,
		            ...
		            try {
		            	...
		            	List<ModelInternet> listBig = cityKpi2019Service.listSpec(mdl);//当前日期大类 必须传日期格式的
		                ...
		                //上月大类
		            	List<ModelInternet> listBig1 = cityKpi2019Service.listSpec(topMonth);//上月大类
		                if(listBig1.size()>0){
		                	for(ModelInternet abo : listBig1) {
			    				String fiveName = abo.getSpec();//手机品牌	
			    				String string = abo.getSaleamount();//数量
...
//			    			 map.put("monthAmount", month2);
		                    ...
		                    //去年同期
		                	List<ModelInternet> listBig2 = cityKpi2019Service.listSpec(netYear);//去年同期大类
		        			if(listBig2.size()>0){
		        				for(ModelInternet abo : listBig2) {
				    				String fiveName = abo.getSpec();//手机品牌	
				    				String string = abo.getSaleamount();//数量
...
//					    			 map.put("yearAmount", year2);
				...
				public Map<String, Object> areaPZ(Locale locale, Model models, HttpServletRequest request,
		            ...
		            try {
		            	...
		            	List<ModelInternet> listBig = cityKpi2019Service.listMemory(mdl);//当前日期大类 必须传日期格式的
		                ...
		                //上月大类
		            	List<ModelInternet> listBig1 = cityKpi2019Service.listMemory(topMonth);//上月大类
		                if(listBig1.size()>0){
		                	for(ModelInternet abo : listBig1) {
			    				String fiveName = abo.getMemory();//手机品牌	
			    				String string = abo.getSaleamount();//数量
...
//			    			 map.put("monthAmount", month2);
		                    ...
		                    //去年同期
		                	List<ModelInternet> listBig2 = cityKpi2019Service.listMemory(netYear);//去年同期大类
		        			if(listBig2.size()>0){
		        				for(ModelInternet abo : listBig2) {
				    				String fiveName = abo.getMemory();//手机品牌	
				    				String string = abo.getSaleamount();//数量
              ...
              //对颜色进行取数
				...
				public Map<String, Object> areaColor(Locale locale, Model models, HttpServletRequest request,
		            ...
		            try {
		            	...
		            	List<ModelInternet> listBig = cityKpi2019Service.listColor(mdl);//当前日期大类 必须传日期格式的
		                ...
		                //上月大类
		            	List<ModelInternet> listBig1 = cityKpi2019Service.listColor(topMonth);//上月大类
		                if(listBig1.size()>0){
		                	for(ModelInternet abo : listBig1) {
			    				String fiveName = abo.getColor();//手机品牌	
			    				String string = abo.getSaleamount();//数量
...
//			    			 map.put("monthAmount", month2);
		                    ...
		                    //去年同期
		                	List<ModelInternet> listBig2 = cityKpi2019Service.listColor(netYear);//去年同期大类
		        			if(listBig2.size()>0){
		        				for(ModelInternet abo : listBig2) {
				    				String fiveName = abo.getColor();//手机品牌	
				    				String string = abo.getSaleamount();//数量
  ...
  //对渠道进行取数
				...
				public Map<String, Object> areaClient(Locale locale, Model models, HttpServletRequest request,
		            ...
		            try {
		            	...
		            	List<ModelInternet> listBig = cityKpi2019Service.listClientName(mdl);//当前日期大类 必须传日期格式的
		                ...
		                //上月大类
		            	List<ModelInternet> listBig1 = cityKpi2019Service.listClientName(topMonth);//上月大类
		                if(listBig1.size()>0){
		                	for(ModelInternet abo : listBig1) {
			    				String fiveName = abo.getClientname();//手机品牌	
			    				String string = abo.getSaleamount();//数量
...
//			    			 map.put("monthAmount", month2);
		                    ...
		                    //去年同期
		                	List<ModelInternet> listBig2 = cityKpi2019Service.listClientName(netYear);//去年同期大类
		        			if(listBig2.size()>0){
		        				for(ModelInternet abo : listBig2) {
				    				String fiveName = abo.getClientname();//手机品牌	
				    				String string = abo.getSaleamount();//数量
				...
				public List<StockTurnOver> doSomeThings(HttpServletRequest request,HttpServletResponse response) throws IOException, ParseException  {
		          ...
		          // String nextYear = format.format(dateYear);
		           ...
		           if(qq==null || "".equals(qq)){
		        	   String time ="20200101";
		        	   listAllStock = cityKpi2019Service.listAllStock(time);  
		           }
		           if(qq!=null && !"".equals(qq)){
