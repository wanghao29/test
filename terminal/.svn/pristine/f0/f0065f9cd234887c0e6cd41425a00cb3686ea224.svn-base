...
function addShape(rowType,cusChan,idx){
	   if(cusChan.showType!=0 && cusChan.showType!=1){
		   ...
		   $("#"+rowType).append('<li id="'+rowType+'_'+idx+'" class="rowType '+rowType+'" cusChanId="'+cusChan.id+'"><div class="box"><div class="box-head"><div class="title" title1="'+tip+'">'+cusChan.cnName+'</div></div>'+
	      ...
	      if(cusChan.showType==2){ //曲线图
			   ...
			   //单位：万
			   if(wanMore(value)&&cusChan.cnName!="渠道销售平均DOU" && cusChan.cnName!="提货数量" 
				   && cusChan.cnName!="渠道销售平均ARPU" && cusChan.cnName!="渠道销售平均MOU" && cusChan.cnName!="分销量" 
					   && cusChan.cnName!="终端公司零售量" && cusChan.cnName!="总零售量"){
				   unit = "万"+unit;
			   ...
			   }else if(String(sum).indexOf(".")!=-1 && cusChan.cnName!="渠道销售平均DOU" && cusChan.cnName!="提货数量" 
				   && cusChan.cnName!="渠道销售平均ARPU" && cusChan.cnName!="渠道销售平均MOU" && cusChan.cnName!="分销量"
				   && cusChan.cnName!="终端公司零售量" && cusChan.cnName!="总零售量"){
				   sum = (sum/1).toFixed(2);
				   option.title.text = "总计："+sum;
				   ...
				   sum = toDecimal2(sum)+unit;
			   option.title.text = "总计："+sum;
			   option.yAxis.axisLabel.formatter = '{value}'+unit;
			   option.series[0].label.formatter = '{c}'+unit;	
			   }else if(cusChan.cnName=="提货数量"){
				   var amount="台";
				   var amount1="台";
				   //sum = toDecimal2(sum);
				   option.title.text = "总计："+sum+amount;
				   option.yAxis.axisLabel.formatter = '{value}'+amount1;
				   option.series[0].label.formatter = '{c}'+amount1;	
				   }
			   else if(cusChan.cnName=="提货频次"){
					   var amount1="次";
					   //sum = toDecimal2(sum)+unit;
					   option.title.text = "总计："+sum+amount1;
					   option.yAxis.axisLabel.formatter = '{value}'+amount1;
					   option.series[0].label.formatter = '{c}'+amount1;	
					   }
			   else if(cusChan.cnName=="渠道销售平均ARPU"){
						   var amount2="元";
						   //sum = toDecimal2(sum)+unit;&& cusChan.cnName!="渠道销售平均MOU"
						 var ss= sum.toFixed(2);
						   option.title.text = "总计："+ss+amount2;
						   option.yAxis.axisLabel.formatter = '{value}'+amount2;
						   option.series[0].label.formatter = '{c}'+amount2;	
						   }
			   else if(cusChan.cnName=="渠道销售平均MOU"){
							   var amount3="分";
							   //sum = toDecimal2(sum)+unit;&& cusChan.cnName!="渠道销售平均MOU"
							   var ss= sum.toFixed(2);
							   option.title.text = "总计："+ss+amount3;
							   option.yAxis.axisLabel.formatter = '{value}'+amount3;
							   option.series[0].label.formatter = '{c}'+amount3;	
							   }
			   else if(cusChan.cnName=="分销量"){
				   var amount3="台";
				   var amount4="台";
				   //sum = toDecimal2(sum)+unit;&& cusChan.cnName!="渠道销售平均MOU"
				   var ss= sum.toFixed(2);
				   option.title.text = "总计："+ss+amount3;
				   option.yAxis.axisLabel.formatter = '{value}'+amount4;
				   option.series[0].label.formatter = '{c}'+amount4;	
				   }
			   else if(cusChan.cnName=="终端公司零售量"){
				   var amount3="台";
				   //sum = toDecimal2(sum)+unit;&& cusChan.cnName!="渠道销售平均MOU"
				   var ss= sum.toFixed(2);
				   option.title.text = "总计："+ss+amount3;
			   ...
			   $.each(value, function(idx, item) {
				   ...
				   if(cusChan.cnName!="提货数量" && cusChan.cnName!="分销量"){
				   if(wanMore(value)){
...
//				  var value1 = (cusChan.value1).split(',');
//			  }
			   //var value1 = (cusChan.value1).split(',');
			   var option = oneBar();
			   $.each(detail, function(idx, item) {
				   option.xAxis[0].data.push(item);
				  // option.yAxis[0].data.push(value1);
				  // item = {value:value1,label:{formatter:item}};
			   });
			   ...
			   if(isNormal&&cusChan.cnName!="渠道销售平均DOU"){
				   var unit = isEmptyObject(cusChan.unit)?"":cusChan.unit;	
