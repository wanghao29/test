...
public class PortrayIntegration {
		...
		for(int i=1; i<months.size(); i++){
			...
			if(mapListMonths.contains(month)){				
	        	for(CusChanPortray portray : portrays){
	        		...
	        		try{
	        			if(portray.getCnName().equals("单品促销能力") && !resultSet.get(j).get("single_sales_amount").toString().equals("")) {
		        			String vv =resultSet.get(i).get("single_sales_amount").toString();
	...
	public static void showType4(Map<String,Object> resultSet,List<CusChanPortray> portrays){	
		for(CusChanPortray portray : portrays){
			...
			for( int i =0 ; i<nameStr.split(",").length ; i++){
				String del  = nameStr.split(",")[i];
				String val = (resultSet==null || resultSet.size()==0)?"0":(resultSet.get(del)==null?"0":resultSet.get(del).toString());
				
				//value += val + ",";
				if(i< detailList[0].split(",").length) {
					map.put( detailList[0].split(",")[i], Double.valueOf(val));
				}
	        ...
	        if(map.containsKey("广州")) {
	            ...
	            for(Map.Entry<String,Double> mapping:list){ 
		              // System.out.println(mapping.getKey()+":"+mapping.getValue()); 
		               ...
		               cnNameStr += mapping.getKey() + ",";
		          } 
	            value = value.substring(0, value.length()-1);
				cnNameStr = cnNameStr.substring(0, cnNameStr.length()-1);
				portray.setValue(value);
				portray.setDetail(cnNameStr);
	        }else {
	        	...
	        	portray.setValue(value);
				portray.setDetail(detailList[0]);
	        }
	...
	public static void showType6(List<Map<String,Object>> mapList,List<CusChanPortray> portrays){	
		for(CusChanPortray por : portrays){
			...
			for(Map<String,Object> map : mapList){
				...
				String[] brand= {};
				String[] count= {};
				if(brandStr.indexOf(" ")!=-1 && !brandStr.contains(",")){
					 brand = brandStr.split("\\s+");
					
					}else {
						brand = brandStr.split(",");	
						
					}
				   if(countStr.indexOf(" ")!=-1){
					count = countStr.split("\\s+");
				/*	for (String string : count) {
					 ...
					 for (int i = 0; i < count.length;i++ )
				        {
						 count[i] = count[i].replace("未知","0");
				        }
					
					
										
					}else {
						count = countStr.split(",");	
