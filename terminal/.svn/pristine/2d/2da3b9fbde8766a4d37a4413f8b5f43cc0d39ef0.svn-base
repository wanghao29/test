...
<script src="<%=basePath%>resources/js/brandRangeSet/bootstrap-select.js?"></script>
 ...
 <label style="background-color: #00063c;font-size: 18px;margin-left:6%"> 关注维度：</label>  
  ...
  <a download="量数据导出Excel" id="excelOut" class="bt5" href="#" style="height: 50px;margin-left:83%">导出Excel</a>
   </div>
   </div>
...
<table border="1" bordercolor="#1addfa" id="table5">
           ...
           <%--  <tr  bgcolor="#4682B4">
			...
			<td colspan="18"  height="40px;"style="font-size: 25px;text-align: center;"> 产品信息操盘量实时数据</td>
			</tr>
		  ...
		  <%--  <c:forEach items="${showAll.newDate}" var="list"> --%>
			...
			<%-- </c:forEach> --%>
			<!-- <tr>
			<td onclick="showAndHide(1)">显示/隐藏</td>
			<td>显示/隐藏</td>
			<td>显示/隐藏</td>
			<td colspan="2">显示/隐藏</td>
			<td colspan="2">显示/隐藏</td>
			<td>显示/隐藏</td>
			<td>显示/隐藏</td>
			<td>显示/隐藏</td>
			<td>显示/隐藏</td>
			<td colspan="3">显示/隐藏</td>
			...
			<tr>
                ...
                <td colspan="2" class="nowXL" align="center">当前销量</td>
                <td rowspan="2" class="DBamount" align="center">对标品销量</td>
                <td rowspan="2" class="JPamount" align="center">竞品销量</td>
                <td colspan="3" class="healthLife" align="center">销量规划健康性</td>
			...
			<tr>			
				...
				<td class="DTamount">累计规划量</td>
				<td class="DTamount">本周规划量</td>
				<td class="nowXL">累计销量</td>
				<td class="nowXL">本周销量</td>
...
if($('#startBZ').attr('checked') == 'checked'){
		   
	$(".BZstart").hide();
...
if($('#ZQlife').attr('checked') == 'checked'){
		  
	$(".lifeZQ").hide();
...
if($('#amountDT').attr('checked') == 'checked'){
		   
	$(".DTamount").hide();
...
if($('#amountDB').attr('checked') == 'checked'){
		   
	$(".DBamount").hide();
...
if($('#amountJP').attr('checked') == 'checked'){
		   
	$(".JPamount").hide();
...
if($('#healthXL').attr('checked') == 'checked'){
		   
	$(".healthLife").hide();
