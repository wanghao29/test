...
@Controller
@RequestMapping("/operateIncome")
public class OperateIncomeController {
	@Resource
	private IOperateIncomeaService operateIncomeaService;
   	...
   	public String newIndex(HttpServletRequest request, Locale locale, Model model, String special, String billcyc) {
		...
		return "gfKpi/operateIncome/operateIncome";
   	}
	...
	public JSONObject query(HttpServletRequest req, HttpServletResponse res) {
		...
		try {
			operateIncomeaService.calculatedScore(String.valueOf(params.get("startMonth")), String.valueOf(params.get("endMonth")));
			List<OperateIncome> outList = new ArrayList<OperateIncome>();
			 PageInfo<OperateIncome> pageInfo =operateIncomeaService.queryData(params);
