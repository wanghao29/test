...
<head>
	...
	<script type="text/javascript">
		$(function(){
			...
			//初始化列表
			loadTable('#grid',
		            basePath + "/amount/getTeamList.do",
		            ['month','city','type','value'],
		            [ '月份', '地市', '收入类型','收入值'],
		            true);
			//查询
			$("#btnSearch").click(function(){
				...
				if($("#typeSelect").val() != null && $("#typeSelect").val() != ""){
					$("#search_hid").val(1);
				}
    ...
    $(table).bootstrapTable({
        ...
        onEditableSave: function (field, row, oldValue, $el) {
            $.ajax({
                type: "post",
                url: basePath + "/amount/updateByTeam.do",
	...
	function createCols(params,titles) {
        ...
        for(var i = 0;i<params.length;i++) {
            var obj = {};
            if(titles[i] == '收入值'){
                obj.editable={type: 'text', title: '编辑', validate:  function (v) {if(v=='0'){return '收入值不能修改为0';}}}
            }
	...
	function addSubmit(){
		if(checkSubmit()){
			$.ajax({
	              type: "post",
	              url: basePath + "/amount/addTeam.do",
	              contentType: "application/json; charset=utf-8",
	              data: JSON.stringify({'addMonth':$('#addMonth').val(),'addType':$('#addType').val(),'addCity':$('#addCity').val(),'addValue':$('#addValue').val()}),
	              dataType: 'JSON',
	              success: function (data, status) {
	                  if (status == "success") {
	                      if(!confirm("是否继续新增")){
...
<div id="box_list">
	<form class="form-horizontal" id="form_dialog" method="post" >
		...
		<div style = "float:left; width:150px;">
			<select class="select" style="width: 150px;color:blue;" id="typeSelect">
														<option value="" >请选择收入类型</option>
				 	 				   		       		<option value="保内收入">保内收入</option>
				 	 				   		       		<option value="非保收入">非保收入</option>
				 	 				   		       		<option value="外采收入">外采收入</option>
				 	 				   		       		<option value="手机保障收入">手机保障收入</option>
				 	 				   		       		<option value="终端配件销售收入">终端配件销售收入</option>
				 	 				   		       		<option value="地市移动项目补贴收入">地市移动项目补贴收入</option>
				 	 				   		       		<option value="总部补贴">总部补贴</option>
				 	 				   		       		<option value="互联网运营中心补贴">互联网运营中心补贴</option>
				 	 				   		       		<option value="能力拓展收入">能力拓展收入</option>
				 	 				   		       		<option value="网点管理费">网点管理费</option>
				 	 				   		       		<option value="酬金">酬金</option>
			</select>
		</div>
		<div style="float:left;width:150px;">
	...
	<!-- '月份', '地市', '收入类型','收入值' -->
		...
		<br><br>
		<label style="color:blue;">售后团队收入类型:</label>
		<select class="select" style="width: 30%;color:blue;" id="addType">
														<option value="" >请选择收入类型</option>
				 	 				   		       		<option value="保内收入">保内收入</option>
				 	 				   		       		<option value="非保收入">非保收入</option>
				 	 				   		       		<option value="外采收入">外采收入</option>
		...
		<br><br>
		<label style="color:blue;float:left;">收入值:</label>
