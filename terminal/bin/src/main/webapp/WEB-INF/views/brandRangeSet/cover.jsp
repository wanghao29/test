 ...
 <label style="background-color: #00063c;font-size: 18px;"> 关注维度：</label>  
   ...
   <input id="spec2" type="hidden" value="${map.COMPETSPEC2}">
    <select id="select1" style="background-color: #00063c;font-size: 18px;">
        ...
        <option>收</option>
        <option>价</option>
        <option selected="selected">覆</option>
        <option>库</option>
        <option>份</option>
        <option>秩</option>
   ...
   <a download="table导出Excel" id="excelOut" class="bt5" href="#" style="height: 50px;">导出Excel</a>
   </div>
   </div>
...
<table border="1" bordercolor="#1addfa" id="table5">
			...
			<tr  bgcolor="#4682B4">
			<td colspan="18" align="center" height="40px;"style="font-size: 25px;">产品信息操盘覆实时数据</td>
			</tr>
			...
			<tr>
                ...
                <td colspan="2" align="center">动态规划量（台）</td>
                <td colspan="2" align="center">当前覆盖量</td>
                <td rowspan="2" align="center">对标品覆盖量</td>
                <td rowspan="2" align="center">竞品覆盖量</td>
  ...
  <tr>	
			    ...
			    <td>${list.totalPlan}</td>	
			    <td>${list.nowPlan}</td>					
				<td>${list.nowSale}</td>
				<td>${list.weekSale}</td>
				<td>${list.benchSale}</td>
				<td>${list.competitorSale}</td>
			    ...
			    <fmt:formatNumber type="number" maxFractionDigits="0">${list.benchPlan}%
			 	...
			 	<c:if test="${list.benchPlan>30}">
			 	<td style="color: red;"><fmt:formatNumber type="number" value="${list.benchPlan}" pattern="#.00"/>
			 	%
			 	</td>
			 	</c:if> 
			 	<c:if test="${list.benchPlan>0&&list.benchPlan<30}">
			 	<td style="color: yellow;"><fmt:formatNumber type="number" value="${list.benchPlan}" pattern="#.00"/>
			 	%</td>
			 	</c:if>
			 	<c:if test="${list.benchPlan<0}">
			 	<td style="color: #32CD32;"><fmt:formatNumber type="number" value="${list.benchPlan}" pattern="#.00"/>
			 	%</td>
			 	</c:if>  
				<c:if test="${ list.benchPlan==0}">
				...
				<c:if test="${list.benching>30}">
			 	<td style="color: red;"><fmt:formatNumber type="number" value="${list.benching}" pattern="#.00"/>
			 	%
			 	</td>
			 	</c:if> 
			 	<c:if test="${list.benching>0&&list.benching<30}">
			 	<td style="color: yellow;"><fmt:formatNumber type="number" value="${list.benching}" pattern="#.00"/>
			 	%</td>
			 	</c:if>
			 	<c:if test="${list.benching<0}">
			 	<td style="color: #32CD32;"><fmt:formatNumber type="number" value="${list.benching}" pattern="#.00"/>
			 	%</td>
			 	</c:if>  
				<c:if test="${list.benching==0}">
                ...
                <c:if test="${list.benchCompetitor>30}">
			 	<td style="color: red;"><fmt:formatNumber type="number" value="${list.benchCompetitor}" pattern="#.00"/>
			 	%
			 	</td>
			 	</c:if> 
			 	<c:if test="${list.benchCompetitor>0&&list.benchCompetitor<30}">
			 	<td style="color: yellow;"><fmt:formatNumber type="number" value="${list.benchCompetitor}" pattern="#.00"/>
			 	%</td>
			 	</c:if>
			 	<c:if test="${list.benchCompetitor<0}">
			 	<td style="color: #32CD32;"><fmt:formatNumber type="number" value="${list.benchCompetitor}" pattern="#.00"/>
			 	%</td>
			 	</c:if>  
               <c:if test="${list.benchCompetitor==0}">
