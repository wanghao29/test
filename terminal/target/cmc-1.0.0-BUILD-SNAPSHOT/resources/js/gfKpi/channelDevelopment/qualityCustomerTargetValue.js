...
function jqgridInit(){
	  jQuery("#grid").jqGrid(
	      {
	        url:basePath+"/qualityCustomerTargetValue/queryData.do",
	        mtype:'post',
	        datatype: "json",
	        ...
	        //scroll:1, 滚动加载
	        //datatype : "local",
	        colNames : [ '月份','优质客户数量','目标值','完全情况占比','时间进度','得分' ],
	        colModel : [ 
	           {name : 'billcyc',index : 'billcyc' }, 
			   {name : 'yzkhsl',index : 'yzkhsl', formatter: "number", formatoptions: {thousandsSeparator:",", defaulValue:"",decimalPlaces:2}  }, 
               {name : 'mbz',index : 'mbz', formatter: "number", formatoptions: {thousandsSeparator:",", defaulValue:"",decimalPlaces:2}  }, 
			   {name : 'wqqkzb',index : 'wqqkzb'  }, 
               {name : 'sjjd',index : 'sjjd'  }, 
               {name : 'df',index : 'df' } 
               ],
...
function assignLableValue(rowData){
	...
	$("#wcd").text(rowData.wqqkzb);
	$("#sjjd").text(rowData.sjjd);
	$("#df").text(rowData.df);

	if(Number(rowData.sjjd) < 0){
		$("#sjjd").addClass("text-danger");
	}else{
		$("#sjjd").removeClass("text-danger");
	}
	}
