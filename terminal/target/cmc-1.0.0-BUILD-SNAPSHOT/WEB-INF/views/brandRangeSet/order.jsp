...
<script src="<%=basePath%>resources/js/brandRangeSet/bootstrap-select.js?"></script>
 ...
 <label style="background-color: #00063c;font-size: 18px;margin-left:19%"> 关注维度：</label>  
   ...
   <input id="spec2" type="hidden" value="${map.COMPETSPEC2}">
    <select id="select1" style="background-color: #00063c;font-size: 18px;">
        ...
        <option>库</option>
        <option>份</option>
        <option selected="selected">秩</option>
   ...
   <a download="秩数据导出Excel" id="excelOut" class="bt5" href="#" style="height: 50px;margin-left:70%">导出Excel</a>
   </div>
   </div>
...
<table border="1" bordercolor="#1addfa" id="table5">
			...
			<tr  bgcolor="#4682B4">
			<td colspan="12" align="center" height="40px;"style="font-size: 25px;">产品信息操盘秩实时数据</td>
			</tr>
			<tr  bgcolor="#4682B4">
			...
			<tr>
                ...
                <td rowspan="2" class="healthLife">DM本地激活率</td>
				<td rowspan="2" class="healthLife">DM异地激活率</td>
				<td rowspan="2" class="healthLife">疑似套机率</td>
  ...
  <tr>	
		       <%--  <td>${list.brandname}</td>
			    ...
			    <fmt:formatNumber type="number" maxFractionDigits="0">${list.benchPlan}%
			 	...
			 	<td class="healthLife" style="color: red;"><fmt:formatNumber type="number" value="${list.weekSale}" pattern="#.00"/>
			 	%
			 	</td>
			 	</c:if> 
			 	<c:if test="${list.weekSale>0&&list.weekSale<30}">
			 	<td class="healthLife" style="color: yellow;"><fmt:formatNumber type="number" value="${list.weekSale}" pattern="#.00"/>
			 	%</td>
			 	</c:if>
			 	<c:if test="${list.weekSale<0}">
			 	<td class="healthLife" style="color: #32CD32;"><fmt:formatNumber type="number" value="${list.weekSale}" pattern="#.00"/>
			 	%</td>
			 	</c:if>  
				<c:if test="${ list.weekSale==0}">
				...
				<c:if test="${list.benchSale>30}">
			 	<td class="healthLife" style="color: red;"><fmt:formatNumber type="number" value="${list.benchSale}" pattern="#.00"/>
			 	%
			 	</td>
			 	</c:if> 
			 	<c:if test="${list.benchSale>0&&list.benchSale<30}">
			 	<td class="healthLife" style="color: yellow;"><fmt:formatNumber type="number" value="${list.benchSale}" pattern="#.00"/>
			 	%</td>
			 	</c:if>
			 	<c:if test="${list.benchSale<0}">
			 	<td class="healthLife" style="color: #32CD32;"><fmt:formatNumber type="number" value="${list.benchSale}" pattern="#.00"/>
			 	%</td>
			 	</c:if>  
				<c:if test="${list.benchSale==0}">
                ...
                <c:if test="${list.competitorSale>30}">
			 	<td class="healthLife" style="color: red;"><fmt:formatNumber type="number" value="${list.competitorSale}" pattern="#.00"/>
			 	%
			 	</td>
			 	</c:if> 
			 	<c:if test="${list.competitorSale>0&&list.competitorSale<30}">
			 	<td class="healthLife" style="color: yellow;"><fmt:formatNumber type="number" value="${list.competitorSale}" pattern="#.00"/>
			 	%</td>
			 	</c:if>
			 	<c:if test="${list.competitorSale<0}">
			 	<td class="healthLife" style="color: #32CD32;"><fmt:formatNumber type="number" value="${list.competitorSale}" pattern="#.00"/>
			 	%</td>
			 	</c:if>  
               <c:if test="${list.competitorSale==0}">
